...
const expected = JSON.parse(fs.readFileSync(
  path.join(repository, 'tests/conformance/expected/p8scii.json'),
  'utf8',
))

test('production Wasm emits a deterministic source-bound P8SCII candidate', () => {
  const directory = path.join(repository, 'captures/official', `public-wasm-p8scii-${process.pid}-${Date.now()}`)
  try {
    ...
    const captures = outputs.map((output) => {
      const result = spawnSync('corepack', [
        ...
        '--cart', 'tests/conformance/probes/p8scii.p8', '--output', output,
