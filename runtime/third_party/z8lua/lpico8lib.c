    ...
    switch (lua_type(l, 1))
    {
        case LUA_TSTRING: {
            ...
            if (flags & 0x1) {
                ...
                uint32_t bits = strtoul(buffer, NULL, 16);
                // With flags 0x3 the hexadecimal word is already the raw
                // signed 16:16 representation (e.g. 1234abcd -> 0x1234.abcd).
                lua_pushnumber(l, flags & 0x2 ? lua_Number::frombits(bits)
                                              : (lua_Number)bits);
                return 1;
            }
            else if (flags & 0x2) {
