...
| API-HOST-001 | `apps/web/src/runtime/platform-host.ts` and `apps/mobile/android/app/src/main/java/dev/aico8/research/MainActivity.java` | Deduplicated lifecycle suspension, native audio focus, logical input, persistence, clock, and bounded platform services without gameplay ownership | Browser Web/PWA, Android WebView, later Linux Web shell |
...
| DATA-VALIDATION-001 | Exit results, platform/build identities, diffs, evidence links, same-build static/temporal source-HD review boundaries, immutable human decision lineage, measured release budgets, active-browser layout measurements/screenshots, and fail-closed named Android physical-device evidence for every target profile | `specs/schemas/hd-review-packet-v1.schema.json`, `specs/schemas/hd-review-decision-v1.schema.json`, `specs/schemas/release-validation-v1.schema.json`, `specs/schemas/android-device-validation-v2.schema.json`, `specs/schemas/android-device-manual-decision-v1.schema.json`, and domain validators |
