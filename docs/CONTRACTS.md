...
| API-HOST-001 | `apps/web/src/runtime/platform-host.ts`, `apps/mobile/src/linux-handheld-capture.ts`, and `apps/mobile/android/app/src/main/java/dev/aico8/research/MainActivity.java` | Deduplicated lifecycle suspension, native audio focus, logical input, persistence, clock, and bounded platform services without gameplay ownership; Linux stays browser-first and may propose a shell only from an evidence-bound failed capability | Browser Web/PWA, Android WebView, Linux handheld Web, later measured-gap shell |
...
| DATA-TYPOGRAPHY-A11Y-001 | Source-bound language/script coverage, CSS-pixel floors, WCAG contrast, delivery-profile fit, source-derived assistive descriptions, compatibility-state neutrality, regression rejection, and an independent human readability decision bound to the exact pending audit and review packet | `specs/schemas/typography-accessibility-audit-v1.schema.json`, `specs/schemas/typography-readability-decision-v1.schema.json`, and `packages/contracts/src/typography-accessibility.ts` |
...
| DATA-LINUX-SIMULATOR-001 | Exact public Web tree, asset manifest, entry module, Linux target, Ubuntu/X11 Chromium and explicit SwiftShader identity, 1024-square screenshot, product-path virtual-controller masks, offline/storage/lifecycle/capability artifacts, startup, and full 60-second frame-budget evidence; status is derived and every retained hash is independently recomputed | `specs/schemas/linux-handheld-simulator-validation-v1.schema.json` and `packages/contracts/src/linux-handheld-simulator-validation.ts` |
| DATA-VALIDATION-001 | Exit results, platform/build identities, diffs, evidence links, same-build static/temporal source-HD review boundaries, immutable human decision lineage, measured release budgets, active-browser layout measurements/screenshots, reproducible named Android 1024-square emulator host evidence plus same-staged-tree 1024-square Chromium performance simulation, required Linux 1024-square simulator evidence, optional fail-closed physical-device evidence, and named Linux browser evidence whose capability gaps exactly govern shell eligibility | `specs/schemas/hd-review-packet-v1.schema.json`, `specs/schemas/hd-review-decision-v1.schema.json`, `specs/schemas/release-validation-v1.schema.json`, `specs/schemas/android-device-validation-v2.schema.json`, `specs/schemas/android-device-manual-decision-v1.schema.json`, `specs/schemas/linux-handheld-simulator-validation-v1.schema.json`, `specs/schemas/linux-handheld-validation-v1.schema.json`, `specs/schemas/linux-handheld-manual-decision-v1.schema.json`, and domain validators |
...
| --- | --- | --- | --- |
| JOB-INGEST-001 | DATA-CART-001 | DATA-WORKSPACE-001 | `packages/ingest` and `scripts/ingest-cart.ts` decode losslessly, preserve absent/present sections and shared-memory aliases, bind provenance, reject divergent dual edits, and prove exact ROM rebuild equality before downstream use |
