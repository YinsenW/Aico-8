...
outside the current delivery roadmap. Android completion uses retained CI evidence from the named API 35, 1024×1024 emulator: simulated controller and audio-focus paths, lifecycle, offline, orientation, storage, touch, startup, and a 60-second frame budget. Physical hardware is a non-blocking compatibility follow-up.
