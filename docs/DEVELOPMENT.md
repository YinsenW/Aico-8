...
`pnpm verify:android-host` validates the Android target profile, exact Web-artifact inventory and tree identity, Capacitor/native policy, lifecycle/audio-focus bridge, external signing boundary, and source/staging/native-asset drift rejection. The `android-host` CI job additionally performs clean Java 21/API 36 debug and externally signed research APK/AAB builds, then boots the named API 35 emulator at a verified 1024×1024 display. The emulator must pass offline local-host, lifecycle/resume, DOM touch, persistent storage, orientation, simulated DPAD delivery, simulated audio-focus loss/recovery, and startup. Before native packaging, `pnpm capture:android-web-simulator` runs the same staged Web bytes for 60 seconds in a 1024×1024 Linux Chromium simulator and enforces the unchanged Android target frame budget. The retained emulator and same-build simulator evidence form one completion bundle; an unaccelerated cloud emulator's SwiftShader rate is retained only as infrastructure diagnosis, never accepted as a device-performance measurement.
Physical hardware is an optional compatibility probe. When it is available, use `pnpm --filter @aico8/mobile capture:device -- <debug-apk> <android-test-apk> <android-web-lineage.json> <target-profile.json> <device-profile-id> <controller-name> <performance-seconds> <output-directory>` with exactly one authorized non-emulator device. Before touching the device, the collector opens the APK and proves every packaged `assets/public/` byte against the supplied lineage. It hashes the private serial, requires airplane mode plus disabled Wi-Fi and mobile data, and restores the pre-capture network state on success or failure. `verify:device` and `finalize:device` retain a fail-closed optional physical record; a missing physical record does not reopen or block an emulator-verified Android exit.
`pnpm verify:linux-handheld` validates the browser-first target, simulator/device contracts, shared lifecycle/audio paths, and the measured-gap shell prohibition. The required `linux-handheld` CI job builds the public Web host and runs `pnpm capture:linux-web-simulator` inside Ubuntu/Xvfb with a fresh, foreground 1024×1024 Chromium profile. The collector binds the complete Web tree, asset manifest, entry module, target bytes, environment, screenshot, service-worker offline reload, storage round trip, product-adapter controller masks, audio unlock, fullscreen, lifecycle freeze/resume, Wasm, startup, and every retained post-warmup frame from a 60-second window. A second fresh browser loads the same product tree solely to prove WebGL2 through the named SwiftShader backend; this isolates deterministic capability discovery from product-path frame timing, because forcing a software graphics backend on the performance browser would measure the probe's compositor cost. Both sessions must use the same Chrome build, Xvfb mode, and 1024-square viewport, and their facts are bound into one capability report. `pnpm verify:linux-web-simulator` independently recomputes every byte/hash, PNG dimension, behavior result, derived budget, and Xvfb identity. Only a passed retained CI bundle can verify the Linux exit. Physical Linux hardware is optional compatibility evidence. When available, `capture:linux` binds the exact Web release tree, Linux target bytes, seven retained evidence files, browser/device identity, capability outcomes, and at least 60 seconds of frame samples. Every capability gap must cite its designated artifact hash; `verify:linux` recomputes every hash and `finalize:linux` accepts only a content-bound four-check human decision. `pending-human` and `browser-gap` never replace or invalidate the required simulator result; the latter only permits a thin-shell investigation for its exact failed capabilities.
...
`pnpm verify:typography` validates routing, font evidence, contours, generated metrics, fit goldens, resource hashes, OS independence, and public completeness/accessibility mutations; `pnpm verify:text-completeness-private` binds the private full replay/probes. `pnpm verify:typography-accessibility-private` binds the rebuilt private package to declared CSS-pixel floors, contrast, delivery-profile layout, language coverage, source-derived assistive descriptions, zero compatibility mutations, and an independent human readability decision. A reviewer records all four checks explicitly with `pnpm accept:typography-readability-private --workspace <workspace> --reviewer <id> --phone-title <passed|failed> --hud-labels <passed|failed> --glyphs <passed|failed> --hierarchy <passed|failed> --notes <text> --write true`; the command archives the exact pending audit and review packet, derives approval/rejection from the four verdicts, and writes an immutable decision. The private verifier reconstructs the final audit and rejects missing, forged, transplanted, or drifted review evidence. If a deterministic rebuild recreates the exact pending audit and packet, the same command replays matching immutable decision bytes; subject, reviewer, verdict, or notes drift requires a new review.
