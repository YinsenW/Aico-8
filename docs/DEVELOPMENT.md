...
Use `pnpm --filter @aico8/mobile capture:device -- <debug-apk> <android-test-apk> <android-web-lineage.json> <target-profile.json> <device-profile-id> <controller-name> <performance-seconds> <output-directory>` with exactly one authorized non-emulator device. Before touching the device, the collector opens the APK and proves every packaged `assets/public/` byte against the supplied lineage. It hashes the private serial, requires airplane mode plus disabled Wi-Fi and mobile data, proves user-orientation state preservation, and measures at least 60 seconds of named-controller play against the lineage-bound target profile's frame sample and budgets. The device's pre-capture network state is restored on success or failure. Before review or handoff, run `pnpm --filter @aico8/mobile verify:device -- <android-device-validation.json> <debug-apk> <android-web-lineage.json> <target-profile.json> <evidence-directory>`; it independently repeats the APK-content proof and recomputes all subject and six retained artifact hashes without requiring the device to remain attached. The report remains `pending-human` until `finalize:device` consumes a content-bound reviewer decision for audio interruption/recovery, controller gameplay, vendor-WebView gameplay, and sustained quality. A reviewed report must then be reverified with its exact pending-report and manual-decision bytes appended to `verify:device`; missing, forged, or transplanted human evidence is rejected. Implementation or machine capture alone cannot verify the exit.
