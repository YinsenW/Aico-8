...
`pnpm verify:linux-handheld` validates the browser-first Linux target, device-evidence and manual-decision contracts, shared lifecycle/audio paths, and the measured-gap shell prohibition. On a named device, `capture:linux` binds the exact Web release tree, Linux target bytes, seven retained evidence files, browser/device identity, capability outcomes, and at least 60 seconds of frame samples. Every capability gap must cite the hash of its designated retained artifact: controller uses `controller.json`, offline uses `offline.json`, storage uses `storage.json`, and audio/fullscreen/Wasm/WebGL2 use `capabilities.json`; an arbitrary well-formed hash is rejected. `verify:linux` recomputes every hash; `finalize:linux` accepts only a content-bound four-check human decision. A reviewed report must be reverified with its exact pending-report and decision bytes appended to `verify:linux`, which reconstructs and compares the entire final report. `pending-human` and `browser-gap` are incomplete states: the former still needs play review, while the latter permits investigation of a thin shell only for the exact failed capabilities and does not verify the exit.
