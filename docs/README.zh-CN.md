...
Aico 8 不绑定某个模型或编码客户端。默认发布物 `aico8-remake.zip` 遵循
Agent Skills 的 `SKILL.md` 约定。同一个 ZIP 可以用于 Claude Code、Codex、
OpenCode 和 Cherry Studio，区别只有首次导入的位置：

1. 从 [Releases](https://github.com/YinsenW/Aico-8/releases/latest) 下载最新的
   `aico8-remake.zip`。
2. 按客户端安装：

| Agent 客户端 | 安装同一个 ZIP |
| --- | --- |
| [Claude Code](https://code.claude.com/docs/en/slash-commands#where-skills-live) | 解压到 `~/.claude/skills/aico8-remake/` |
| Codex | 解压到 `~/.agents/skills/aico8-remake/` |
| [OpenCode](https://opencode.ai/docs/skills/#place-files) | 解压到 `~/.config/opencode/skills/aico8-remake/`；它也能识别前两个目录 |
| [Cherry Studio](https://docs.cherry-ai.com/docs/en-us/advanced-basic/agent) | 资源库 → 添加 Skill → 本地导入 → 选择该 ZIP |

Codex 用户也可以选择 Codex 专用的插件外壳：
...
这两行只是 Codex 的便捷安装方式，不是 Aico 8 或通用 Skill 的前提。
...
Agent 会自行加载 Skill、在本机私有目录安装固定版本的引擎、运行浏览器或
Android 模拟器验证，并返回最终文件。普通用户不需要理解仓库路径、构建
