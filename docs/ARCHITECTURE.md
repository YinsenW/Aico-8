...
- Android next: a Capacitor/WebView shell packages the unchanged Web build as
  ...
  declared Capacitor shims may differ. Gradle fails closed without external keys; the named API 35 1024-square emulator is the reproducible completion environment and physical-device runs are non-blocking compatibility probes.
