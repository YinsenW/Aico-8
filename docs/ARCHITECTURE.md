...
- Use authorized official PICO-8 captures as the primary behavioral oracle;
  independent runtimes are diagnostics. Prefer Education Edition for supported 0.2.7 Web probes and record exactly one bounded official cart-load method: `local-cart-file-selection` or `official-drag-drop-data-path`. Use licensed
...
- Linux handhelds use the same content-addressed browser/PWA artifact first. The required acceptance boundary is a fresh Ubuntu/X11 Chromium session at 1024×1024 with an explicitly recorded SwiftShader WebGL renderer: it binds the complete Web tree, manifest, entry module, target bytes, environment, screenshot, product-path controller masks, offline/storage/lifecycle/audio/fullscreen/Wasm/WebGL2 results, startup, and a full 60-second performance window. An independent verifier recomputes every hash and derived result. Named physical-device capture and human play review remain optional compatibility evidence, not completion prerequisites.
