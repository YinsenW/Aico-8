...
async function main() {
  ...
  const engine = JSON.parse(await readFile(path.join(pluginRoot, "engine.json"), "utf8"));
  ...
  try {
    if (!source) {
      ...
      run("git", ["clone", "--depth", "1", "--branch", engine.ref, engine.repository, temporary], pluginRoot);
      source = temporary;
    }
    ...
    const output = run(process.execPath, args, source);
    const result = JSON.parse(output);
    process.stdout.write(`${JSON.stringify({ ...result, pluginVersion: manifest.version, engineRef: engine.ref }, null, 2)}\n`);
  } finally {
